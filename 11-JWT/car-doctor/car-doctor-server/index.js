...
async function run(){
    ...
    try{
        ...
        app.post('/jwt', async(req, res)=>{
            const token = jwt.sign(req.body, process.env.JWT_SECRET, {expiresIn: '1000h'})
        ...
        app.patch('/booking/:id', async(req, res)=>{
            ...
            const result = await bookingCollection.updateOne(query, doc, {upsert: true})
            res.send(result)
        })
        ...
        app.get('/logout', async(req, res)=>{
            res.clearCookie('token', {httpOnly: true}).send({message: 'token cleared'})
        })
    }
    catch(err){
        console.log(err)
